...
Entity entity_create(int id, vec3s pos, Entity_Flags flags) {
  Entity self = {.id = id, .position = pos};
...
void entity_update_ModelMat(Entity *self) {
  ...
  self->modelMat =
      glms_rotate(self->modelMat,
                     glm_rad(self->rotation), (vec3s){{0, 0, 1}});
...
void _update_player(Entity *self) {
  if (window.key[GLFW_KEY_UP].down) {
    if(window.key[GLFW_KEY_UP].pressed) self->velocity = 0;
    ...
    vec3s curOffset = glms_vec3_scale((vec3s){{self->front.x, self->front.y, 0}},
    ...
    vec3s offset = glms_vec3_scale(
        (vec3s){{lastFront.x, lastFront.y, 0}}, self->velocity);
...
void entity_update(Entity *self) {
  ...
  vec2s edgeTwo = glms_vec2_scale((vec2s){{window.size.x + 50, window.size.y + 50}}, self->scale);
